    ...
    function($scope, $http, $location, Const, fyData) {
        ...
        function submit_info(index) {
            if (index === 4) {
                ...
                //邮箱
                if (!checkEmail($scope.temp_info.Email)) {
                    return;
                };
            } else if (index === 3) {
                //身份证
                if (!checkIDCard($scope.temp_info.IDNumber)) {
                    return;
                };
            } else if (index === 7) {
                //邮政编码
                if (!checkZip($scope.temp_info.Zip)) {
                    return;
                };
            };
            ...
            fyData.getToken(function(token) {
                $http({
                    method: 'post',
                    url: Const.baseUrl + 'User/SaveUser?Token=' + token,
                    data: info_data
                }).success(function(req) {
                    // if(1){
        ...
        function create_months(max_month) {
            ...
            for (var i = 1; i <= max_month; i++) {
                if (i < 10) {
                    $scope.month_datas.push('0' + i);
                } else {
                    $scope.month_datas.push(i);
                };
            };
        };
        ...
        function create_days(max_day) {
            ...
            for (var i = 1; i <= max_day; i++) {
                if (i < 10) {
                    $scope.day_datas.push('0' + i);
                } else {
                    $scope.day_datas.push(i);
                };
            };
        };
